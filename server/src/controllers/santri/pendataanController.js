...
exports.updateProfile = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Gagal memperbarui data diri' });
    }
};
...
exports.updatePhoto = async (req, res) => {
    try {
        ...
        if (oldUser.foto_profil) {
            ...
            const oldPath = path.join(__dirname, '../../../public/uploads', oldUser.foto_profil);
...
exports.addOrangTua = async (req, res) => {
    try {
        ...
        if (!nama || !hubungan || !no_hp) {
            return res.status(400).json({ success: false, message: 'Nama, Hubungan, dan No HP wajib diisi' });
        }
        ...
        if (id_user_wali) {
            ...
            if (!parentUser) {
                return res.status(404).json({ success: false, message: 'User wali tidak ditemukan' });
            }
        } 
        // SKENARIO 2: Input Manual (Tidak ada ID User)
            ...
            if (existingUser) {
                return res.status(400).json({ 
                    success: false, 
                    message: 'Nomor HP sudah terdaftar! Silakan gunakan fitur pencarian untuk menghubungkan data.' 
                });
            }
        ...
        if (existingLink) {
            ...
            if (!existingLink.is_active) {
                ...
                return res.json({ success: true, message: 'Data orang tua berhasil diaktifkan kembali.' });
            }
            return res.status(400).json({ success: false, message: 'Orang tua ini sudah terhubung dengan Anda.' });
        }
        ...
        res.json({ 
            success: true, 
            message: id_user_wali ? 'Data berhasil ditautkan.' : 'Data baru berhasil dibuat.' 
        });
