...
exports.getActiveRooms = async (req, res) => {
  try {
    const data = await konsultasiService.listActiveRoomsByTimkes(req.user.id);
...
exports.getHistory = async (req, res) => {
  try {
    const data = await konsultasiService.listRoomHistory({ userId: req.user.id, userRole: 'timkesehatan' });
    return res.json({ success: true, data });
  } catch (error) {
    return handleError(res, error);
  }
...
exports.getHealthSummary = async (req, res) => {
  try {
    const data = await konsultasiService.getRoomHealthSummary({ roomId: req.params.id, userId: req.user.id });
    return res.json({ success: true, data });
  } catch (error) {
    return handleError(res, error);
  }
