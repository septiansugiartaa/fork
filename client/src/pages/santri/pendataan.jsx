    ...
    <div className="min-h-screen bg-gray-50 pb-10 w-full overflow-x-hidden">
      ...
      <div className="bg-[url('../src/assets/header.png')] bg-cover bg-center text-white p-6 pb-24 shadow-lg">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          ...
          <div className="min-w-0"><h1 className="text-2xl font-bold truncate">Edit Profil</h1><p className="text-green-100 text-sm truncate">Kelola informasi pribadi dan akun</p></div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 -mt-16 space-y-6 relative z-10">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <button type="button" onClick={() => setShowPasswordModal(true)} className="text-sm font-medium text-green-600 hover:text-green-800 hover:bg-green-50 px-3 py-2 rounded-lg transition flex items-center"><Lock size={16} className="mr-2" /> Ganti Kata Sandi</button>
          </div>
          <form onSubmit={handleUpdateDataDiri} className="space-y-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Nama Lengkap</label><input type="text" value={dataDiri.nama_lengkap || ''} onChange={(e) => setDataDiri({...dataDiri, nama_lengkap: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none" /></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Tempat Lahir</label><input type="text" value={dataDiri.tempat_lahir || ''} onChange={(e) => setDataDiri({...dataDiri, tempat_lahir: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none" /></div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Tanggal Lahir</label><input type="date" value={dataDiri.tanggal_lahir || ''} onChange={(e) => setDataDiri({...dataDiri, tanggal_lahir: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Email</label><input type="email" value={dataDiri.email || ''} onChange={(e) => setDataDiri({...dataDiri, email: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none" /></div>
            </div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Nomor HP</label><input type="text" value={dataDiri.no_hp || ''} onChange={(e) => setDataDiri({...dataDiri, no_hp: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none" /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Alamat Tinggal</label><textarea rows="3" value={dataDiri.alamat || ''} onChange={(e) => setDataDiri({...dataDiri, alamat: e.target.value})} className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none resize-none" /></div>
          ...
          <div className="block md:hidden space-y-4 w-full">
            {orangTua.map((ortu, idx) => (
              <div key={idx} className="bg-gray-50 border border-gray-200 rounded-xl p-4 flex flex-col gap-3 w-full">
                <div className="flex justify-between items-start gap-2"><div className="min-w-0 flex-1"><h4 className="font-bold text-gray-800 break-words leading-tight">{ortu.nama}</h4><div className="mt-1"><span className="text-xs font-medium bg-green-100 text-green-700 px-2 py-0.5 rounded-full inline-block">{ortu.hubungan}</span></div></div></div>
                ...
                <div className="flex gap-2 pt-2 border-t border-gray-200 mt-1"><button onClick={() => handleEditClick(ortu)} className="flex-1 flex items-center justify-center py-2 text-sm font-medium text-green-600 bg-green-50 hover:bg-green-100 rounded-lg transition"><Edit2 size={14} className="mr-1" /> Edit</button><button onClick={() => handleDeleteOrtu(ortu.id)} className="flex-1 flex items-center justify-center py-2 text-sm font-medium text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition"><Trash2 size={14} className="mr-1" /> Hapus</button></div>
              </div>
            ))}
              ...
              <tbody className="divide-y divide-gray-100">
                {orangTua.map((ortu, idx) => (
                  <tr key={idx} className="hover:bg-gray-50 transition">
                    ...
                    <td className="p-3 text-right space-x-2"><button onClick={() => handleEditClick(ortu)} className="text-green-500 hover:text-green-700 text-sm font-medium inline-flex items-center"><Edit2 size={14} className="mr-1" /> Edit</button><span className="text-gray-300">|</span><button onClick={() => handleDeleteOrtu(ortu.id)} className="text-red-500 hover:text-red-700 text-sm font-medium inline-flex items-center"><Trash2 size={14} className="mr-1" /> Hapus</button></td>
                  </tr>
                ))}
              ...
              {formStep === 2 && (
                <form onSubmit={handleSubmitOrtu} className="space-y-4">
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <button type="submit" disabled={saving} className="flex-1 px-4 py-3 bg-green-600 text-white font-medium rounded-xl hover:bg-green-700 transition disabled:bg-green-300">
