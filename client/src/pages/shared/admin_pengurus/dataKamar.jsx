...
import { Plus, Search, Edit2, Trash2, Users, Loader2, MapPin, Home } from "lucide-react";
...
export default function DataKamarPage({ rolePrefix }) {
  ...
  return (
    <div className="space-y-6 relative">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100 text-gray-600 text-sm uppercase">
                    <th className="p-4 w-[35%]">Nama Kamar</th><th className="p-4 w-[15%]">Kapasitas</th><th className="p-4 w-[15%]">Gender</th><th className="p-4 w-[25%]">Lokasi</th><th className="p-4 text-center w-[10%]">Aksi</th>
                  </tr>
                </thead>
            ...
            {currentData.length > 0 ? currentData.map((item) => (
              <div key={item.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-3">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-bold text-gray-800 text-lg flex items-center gap-2"><Home size={18} className="text-gray-400" /> {item.kamar}</h3>
      ...
      <KamarSantriModal isOpen={modalListSantri.isOpen} onClose={() => setModalListSantri({ ...modalListSantri, isOpen: false })} kamarData={modalListSantri.data} onAssignClick={(kamarData) => setModalAssign({ isOpen: true, data: { kamar: kamarData } })} refreshTrigger={refreshListKey} />
