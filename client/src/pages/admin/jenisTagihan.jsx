...
export default function JenisTagihan() {
  ...
  const fetchJenisTagihan = async () => {
    setLoading(true);
    try {
      const res = await api.get("/admin/jenis-tagihan", { params: { search } });
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/admin/jenis-tagihan/${selectedData.id}`, formData);
        showAlert("success", "Jenis tagihan berhasil diperbarui");
      } else {
        await api.post("/admin/jenis-tagihan", formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/admin/jenis-tagihan/${id}`);
    ...
    <div className="space-y-6 relative">
      ...
      <div className="flex justify-between items-center">
        <div>
            ...
            <p className="text-gray-500 text-sm">Master data untuk jenis-jenis tagihan pesantren.</p>
        </div>
        <button 
            onClick={handleAdd}
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {currentData.length > 0 ? (
                                ...
                                <tr><td colSpan="4" className="p-8 text-center text-gray-500">Data jenis tagihan tidak ditemukan.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                        ...
                        <div key={item.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-3">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <div className="mt-1">
                                        <span className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider">Aktif</span>
                                    </div>
                                </div>
