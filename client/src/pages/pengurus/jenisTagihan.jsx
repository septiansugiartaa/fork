...
export default function JenisTagihan() {
  ...
  const fetchJenisTagihan = async () => {
    setLoading(true);
    try {
      const res = await api.get("/pengurus/jenis-tagihan", {
        params: { search },
      });
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/pengurus/jenis-tagihan/${selectedData.id}`, formData);
        showAlert("success", "Jenis tagihan berhasil diperbarui");
      } else {
        await api.post("/pengurus/jenis-tagihan", formData);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Apakah Anda yakin ingin menghapus jenis ini?")) return;
    try {
      await api.delete(`/pengurus/jenis-tagihan/${id}`);
  ...
  return (
    <div className="space-y-6 relative">
      ...
      {message.text && (
        <div
          className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-[11000] min-w-[320px] max-w-md p-4 rounded-xl shadow-2xl flex items-center gap-3 animate-in slide-in-from-top-5 fade-in duration-300 border-l-4 bg-white ${message.type === "error" ? "bg-white border-red-500 text-red-700" : "bg-white border-green-500 text-green-700"}`}
        >
          <div
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Jenis Tagihan</h1>
          <p className="text-gray-500 text-sm">
            Master data untuk jenis-jenis tagihan pesantren.
          </p>
        </div>
        <button
          onClick={handleAdd}
        ...
        >
          <Plus size={20} />
          <span className="ml-2 hidden md:inline">Tambah Jenis Tagihan</span>
        </button>
      </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100 text-gray-600 text-sm uppercase tracking-wider">
                    <th className="p-4 w-[60%]">Jenis Tagihan</th>
                  ...
                  {currentData.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan="4" className="p-8 text-center text-gray-500">
                        Data jenis tagihan tidak ditemukan.
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            {currentData.length > 0 ? (
              currentData.map((item) => (
                ...
                >
                  ...
                  <button
                    onClick={() => handleEdit(item)}
                    className="mt-1 py-2 bg-green-50 text-green-600 rounded-xl font-semibold text-sm flex justify-center items-center gap-2 active:scale-95 transition"
                  >
