...
export default function Login() {
    ...
    const redirectByRole = (role, replace = false) => {
        if (role === 'santri') navigate('/santri', { replace });
        else if (role === 'orangtua') navigate('/orangtua', { replace });
        else if (role === 'pengurus') navigate('/pengurus', { replace });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // JIKA USER PUNYA BANYAK ROLE
            if (response.data.requireRoleSelection) {
                setAvailableRoles(response.data.availableRoles);
                setTempUserId(response.data.userId);
                setStep(2); // Ganti UI ke layar pilih role
            } 
            // JIKA USER CUMA PUNYA 1 ROLE
            else {
    ...
    const handleRoleSelect = async (selectedRole) => {
        ...
        setError("");
        try {
            const response = await axios.post('http://localhost:3000/api/auth/finalize-login', {
                userId: tempUserId,
            ...
            setStep(1); // Kembalikan ke form awal jika gagal
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleKirimWA = (e) => {
        e.preventDefault();
        if (!forgotData.nama || !forgotData.nip) return alert("Nama dan NIS/NIP wajib diisi!");
        ...
        <div className="min-h-screen flex bg-gradient-to-br from-green-600 to-green-500">
            ...
            <div className="flex w-full lg:w-1/2 justify-center items-center px-4 sm:px-6 lg:bg-white relative z-10">
                ...
                <div className="w-full max-w-lg bg-white/20 backdrop-blur-md lg:bg-transparent lg:backdrop-blur-none p-8 rounded-3xl shadow-lg lg:shadow-none border border-white/20 lg:border-none">
                    ...
                    {/* CONDITIONAL RENDERING: FORM LOGIN (STEP 1) ATAU PILIH ROLE (STEP 2) */}
                    {step === 1 ? (
                        <>
                            <form onSubmit={handleSubmit} className="space-y-5 animate-in fade-in">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input 
                                            // 1. Type dibuat dinamis berdasarkan state
                                            type={showPassword ? "text" : "password"} 
                                            placeholder="Masukkan Kata Sandi" 
                                            // 2. Tambahkan pr-12 (padding-right) agar teks tidak menabrak ikon mata
                                        ...
                                        {/* 3. Tombol Toggle Show/Hide */}
                                        <button
                                            type="button"
                                            onClick={() => setShowPassword(!showPassword)}
                                            disabled={loading}
                                            // Warna ikon dibikin responsif ngikutin tema input-nya (putih di mobile, abu-abu di desktop)
                        ...
                        // UI STEP 2: PILIH ROLE (Gaya Senada dengan Form)
                           ...
                           <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
                               ...
                               <form onSubmit={handleKirimWA} className="p-6 space-y-4">
                                   <div>
                                       ...
                                       <select className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none bg-white text-gray-700" value={forgotData.role} onChange={(e) => setForgotData({...forgotData, role: e.target.value})}>
                                   ...
                                   <div>
                                       ...
                                       <input type="text" placeholder="Contoh: Ahmad Fulan" className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none text-gray-700" value={forgotData.nama} onChange={(e) => setForgotData({...forgotData, nama: e.target.value})} autoFocus />
                                   </div>
                                   <div>
                                       <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">NIS / NIP / No. HP</label>
                                       <input type="text" placeholder="Masukkan identitas terdaftar" className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none text-gray-700" value={forgotData.nip} onChange={(e) => setForgotData({...forgotData, nip: e.target.value})} />
                                   </div>
                                   <div className="pt-4">
