...
export default function InputStafModal({
  isOpen,
  onClose,
  isEditing,
  editData,
  ...
  const initialForm = {
    ...
    no_hp: "",
    jenis_kelamin: "",
    roles: [], // Array string: ['Admin', 'Timkes', dll]
  };

  const [formData, setFormData] = useState(initialForm);

  // Opsi role yang bisa dipilih admin
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.roles.length === 0)
      return alert("Pilih minimal 1 Hak Akses (Role)!");
    onSubmit(formData);
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
          ...
          <form id="stafForm" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                name="jenis_kelamin"
                required
                className="w-full p-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none bg-white"
            ...
            <div className="pt-2 border-t border-gray-100">
              <label className="block text-sm font-bold text-gray-800 mb-2">
                Penugasan Hak Akses (Bisa pilih lebih dari 1)
              </label>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {availableRoles.map((role) => (
                  <label
                    key={role}
                    className={`flex items-center gap-2 p-3 rounded-xl border cursor-pointer transition ${formData.roles.includes(role) ? "bg-blue-50 border-blue-500 text-blue-800 shadow-sm" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50"}`}
                  >
                    <input
                      type="checkbox"
            ...
            {!isEditing && (
              <div className="p-3 bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-xl text-xs font-medium mt-4">
                Info: Password default untuk akun baru adalah{" "}
                <span className="font-bold text-black">12345678</span>
              </div>
            )}
          </form>
        </div>
        ...
        <div className="p-5 border-t border-gray-100 bg-gray-50 rounded-b-2xl flex justify-between items-center">
          {/* Tombol Reset Password Khusus Edit */}
          {isEditing ? (
            <button
              type="button"
              onClick={() => onResetPassword(editData.id)}
              className="px-4 py-2 bg-yellow-100 text-yellow-700 hover:bg-yellow-200 font-semibold text-sm rounded-xl transition flex items-center"
            >
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              type="submit"
              disabled={saving}
              className="px-5 py-2.5 bg-blue-600 text-white font-medium rounded-xl hover:bg-blue-700 transition flex items-center disabled:opacity-70 shadow-lg"
            >
              {saving ? (
