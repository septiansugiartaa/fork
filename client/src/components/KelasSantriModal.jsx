...
export default function KelasSantriModal({ isOpen, onClose, kelasData, onAssignClick, refreshTrigger }) {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState(null); // { id, nama } atau null
  ...
  const fetchSantri = async () => {
    ...
    try {
      ...
      const res = await axios.get(`http://localhost:3000/api/pengurus/kelas/${kelasData.id}/santri`, {
  ...
  // 1. Klik Tombol Sampah -> Buka Modal Konfirmasi
  const handleRemoveClick = (santriId, santriNama) => {
    setDeleteConfirm({ id: santriId, nama: santriNama });
  };

  // 2. Klik "Ya, Hapus" di Modal Konfirmasi -> Eksekusi API
  const confirmDelete = async () => {
    ...
    try {
        const token = localStorage.getItem("token");
        await axios.delete(`http://localhost:3000/api/pengurus/kelas/${kelasData.id}/santri/${deleteConfirm.id}`, {
            headers: { Authorization: `Bearer ${token}` }
        });
        ...
        alert("Gagal menghapus santri"); // Fallback alert error
        console.error(err);
    } finally {
        setIsRemoving(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[85vh] relative z-10">
        ...
        <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
          <div>
            <h3 className="font-bold text-gray-800 text-lg">Daftar Santri</h3>
            <p className="text-sm text-gray-500">Kelas: {kelasData?.kelas}</p>
          </div>
        ...
        <div className="p-4 overflow-y-auto flex-1">
            {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {filteredList.map(santri => (
                        <div key={santri.id} className="flex items-center justify-between px-3 py-2 border border-gray-200 rounded-xl hover:bg-gray-50 transition group">
                            ...
                            <button 
                                onClick={() => handleRemoveClick(santri.id, santri.nama)}
                ...
                <div className="text-center py-10 text-gray-500 italic">Belum ada santri di kelas ini.</div>
            )}
        </div>

        {/* Footer */}
        <div className="p-4 border-t border-gray-100 bg-gray-50 rounded-b-2xl flex justify-end">
            <button onClick={onClose} className="px-5 py-2 bg-white border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-100 transition">Tutup</button>
        </div>
      </div>


      {/* --- MODAL KONFIRMASI KECIL (NESTED) --- */}
      {deleteConfirm && (
        <div className="absolute inset-0 z-[60] flex items-center justify-center bg-black/20 backdrop-blur-[1px] rounded-2xl animate-in fade-in duration-200">
            <div className="bg-white p-6 rounded-2xl shadow-2xl w-80 text-center border border-gray-100 transform scale-100 animate-in zoom-in-95 duration-200">
                ...
                <p className="text-sm text-gray-600 mb-6">
                    Keluarkan <span className="font-bold text-gray-800">{deleteConfirm.nama}</span> dari kelas ini?
                </p>
                <div className="flex gap-3">
                    <button 
                        onClick={() => setDeleteConfirm(null)}
                        className="flex-1 py-2 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 font-medium transition"
                    >
                        Batal
                    </button>
                    <button 
                        ...
                        disabled={isRemoving}
                        className="flex-1 py-2 bg-red-600 text-white rounded-xl hover:bg-red-700 font-medium transition flex items-center justify-center"
                    >
                        {isRemoving ? <Loader2 className="animate-spin" size={18}/> : "Hapus"}
                    </button>
                </div>
            </div>
        </div>
